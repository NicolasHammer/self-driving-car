...
def setJoy_Steer_Throttle(value_steerX, value_throttleX, scale=16384):
    ...
    joystickPosition = vj.generateJoystickPosition(
        wAxisX=xPos_steering, wAxisY=int(scale/2), wAxisZRot=xPos_throttle)
...
def setJoy_Steer_Throttle_Brake(value_steerX, value_throttleX, brake_state, scale=16384):
    ...
    xPos_steering = int(value_steerX*scale)
    xPos_throttle = int(value_throttleX*scale)
    joystickPosition = vj.generateJoystickPosition(wAxisX=xPos_steering, wAxisY=int(
        scale/2), wAxisZRot=xPos_throttle, lButtons=brake_state)
    vj.update(joystickPosition)
